...
class ReversiManager{
	...
	thinkCom(board, turn, availables){
		...
		for(let available of availables){
			...
			const nextAvailables = this._bdManager.getAvailables(nextBoard, nextTurn);
			// MinMax
			const score = this.minmax(nextBoard, nextTurn, nextAvailables, 1);
			if(bestScore < score){
				bestScore = score;
				choise.r = r;
				choise.c = c;
	...
	minmax(board, turn, availables, depth){
		if(T_DEPTH < depth) return 0;// Strength
		//console.log("minmax[" + turn + "]", "Depth:", depth);
		...
		if(!this._bdManager.isAvailable(board, turn)){
			...
			if(!this._bdManager.isAvailable(board, this.nextTurn(turn))){
				...
				const nextTurn = this.nextTurn(turn);
				const nextAvailables = this._bdManager.getAvailables(board, nextTurn);
				return this.minmax(board, nextTurn, nextAvailables, depth);
			}
		}
		// Put disks
		let bestScore = (this._com == turn) ? Infinity*-1:Infinity;
		for(let available of availables){
			const r = available[0];
			const c = available[1];
			const nextBoard = this._bdManager.putDisk(board, turn, r, c);
			...
			const nextAvailables = this._bdManager.getAvailables(nextBoard, nextTurn);
			// MinMax
			const score = this.minmax(nextBoard, nextTurn, nextAvailables, depth+1);
			if(this._you == turn) bestScore = Math.min(bestScore, score);// Min
			if(this._com == turn) bestScore = Math.max(bestScore, score);// Max
		}
		return bestScore;
	}
