...
window.onload = ()=>{
	...
	// Bridge
	const bridgeW = 300;
	createBridge(WIDTH/2-bridgeW/2, HEIGHT/2, WIDTH/2+bridgeW/2, HEIGHT/2, 8);

	// Floor
	const floorL = Bodies.rectangle(0, HEIGHT/2, 170, 20,
			{isStatic: true, chamfer: 0, angle: Math.PI*0.0});
	const floorR = Bodies.rectangle(WIDTH, HEIGHT/2, 170, 20,
			{isStatic: true, chamfer: 0, angle: Math.PI*0.0});
	Composite.add(engine.world, [floorL, floorR]);

	// Car
	createCar(WIDTH/2, 10, 120, 20, 20);
	...
	function createBridge(lX, lY, rX, rY, total){
		...
		const stack = Composites.stack(lX, lY-h/2, total, 1, 0, 0, (x, y)=>{
			return Bodies.rectangle(x, y, w, h, {friction: 1.0, collisionFilter: {group: group}});
		});
		// Chain
		Composites.chain(stack, 0.5, 0, -0.5, 0,
			{stiffness: 1.0, length: 2, render: {type: "line"}});
		// Composite
		const first = 0;
